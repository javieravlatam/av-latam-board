# SIC-AV — Prototipo Funcional del Módulo de Comisiones (v1.6 — arquitectura temporal definitiva)

**CHANGE REQUEST SIC-AV v1.6 (2026-07-13) — Arquitectura temporal definitiva:** se elimina por completo el concepto "Presupuesto del ciclo 26-25". El SIC opera ahora con dos períodos coordinados pero nunca mezclados: el **Período de cobranza** (26→25, sin cambios — determina cobros, facturas cobradas, edad de cartera, tasa base y liquidación) y el **Mes de desempeño** (el último mes calendario cerrado — determina presupuesto, venta neta, cumplimiento, Factor de Presupuesto, IEC, Factor IEC y Bono por Excedente). Ver `TEMPORAL_MODEL_v1.6.md` (fuente técnica oficial), `TEMPORAL_MODEL_AUDIT_v1.6.md` y `CHANGELOG.md` para el detalle completo. **No se ha hecho commit ni push de este cambio** — pendiente de aprobación de Gerencia General.

**v1.5.1 (2026-07-13):** `sic_chile.html` y `sic_peru.html` — el flujo principal al que se llega desde el Portal → SIC AV → País — dejaron de mostrar datos demostrativos. Ahora cargan datos reales del AV LATAM Board a través de `js/sic_data_adapter.js` (mismo adaptador de solo lectura ya usado por `sic_datos_reales.html`), sin duplicar lógica y sin modificar `sic_core.js`. Antes de esta corrección, el acceso principal mostraba datos sintéticos y solo `sic_datos_reales.html` (una vista secundaria) tenía datos reales — ver detalle en `CHANGELOG.md`.
