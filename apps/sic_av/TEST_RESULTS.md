...
- `tests/run_dashboard_real_test.js` — **(v1.5.1)** renderiza `sic_chile.html`/`sic_peru.html` (flujo principal) en un DOM real, confirmando datos reales, aislamiento por país, ausencia de datos demo, textos de estado pendiente exigidos, banner de dos períodos (v1.6) y cero errores de consola.
- `tests/run_v16_temporal_model_tests.js` — **(v1.6)** los 16 casos obligatorios de la sección 14 del CHANGE REQUEST v1.6 (liquidaciones 25 junio/25 julio, Factor Presupuesto e IEC sobre el mismo mes de desempeño, bono sin cobranza, ausencia de presupuesto de ciclo/prorrateo, dashboard/PDF/política mostrando ambos períodos, histórico, año nuevo/bisiesto, meses sin presupuesto/IEC). No requiere servidor HTTP para los casos 1-8 y 12-16 (usa `fs.readFileSync`); los casos 9-11 leen el HTML/JS fuente como texto sin necesidad de jsdom.
